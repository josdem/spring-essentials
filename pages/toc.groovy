"presentacion.html"(title: "Bienvenido al curso : "){
  "presentacion.html#1"(title: "Bienvenido a MakingDevs")
  "presentacion.html#2"(title: "Objetivo del entrenamiento")
  "presentacion.html#3"(title: "Consideraciones")
  "presentacion.html#4"(title: "Herramientas para entrenamiento")
}

"essentials.html"(title: "Elementos esenciales de Springframework"){
  "essentials.html"(title: "¿Qué es lo nuevo en Spring?")
  "essentials.html"(title: "Módulos de Spring"){
    "essentials.html"(title: "Escenarios de uso de Spring"){
      "essentials.html"(title: "Uso típico en una aplicación Web hecha totalmente con Spring")
      "essentials.html"(title: "Uso de Spring en la capa de servicios usando una librería web de terceros")
      "essentials.html"(title: "Uso de elementos remotos")
      "essentials.html"(title: "Intercambiando POJO's existentes")
    }
  }
  "essentials.html"(title: "Mockito")
  "essentials.html"(title: "Diseño de aplicaciones(orientada a interfaces)"){
    "essentials.html"(title: "Creando aplicaciones con baja cohesión y alto acoplamiento")
    "essentials.html"(title: "Alta cohesión y bajo acoplamiento")
  }
}

"ioc.html"(title: "Inversion of control y Dependency Injection"){
  "ioc.html"(title: "Inversión de control"){
    "ioc.html"(title: "Estos principios de diseño debes aprender"){
      "ioc.html"(title: "Acerca de las interdependencias")
      "ioc.html"(title: "Acerca de las implementaciones")
    }
  }
  "ioc.html"(title: "Separación de las interfaces y sus implementaciones"){
    "ioc.html"(title: "Uso de dependencias de forma directa")
  }
}

"spring_container.html"(title: "El conteneder de Spring"){
  "spring_container.html"(title: "Bill of materials")
  "spring_container.html"(title: "Administración de dependencias con Gradle")
  "spring_container.html"(title: "Diferencias entre el BeanFactory y el AppCtx")
  "spring_container.html"(title: "El contenedor de beans de Spring")
  "spring_container.html"(title: "Componentes funcionales y no funcionales")
  "spring_container.html"(title: "Cross cutting concerns")
  "spring_container.html"(title: "Estructura de las clases de dominio")
  "spring_container.html"(title: "Funcionalidad que deseamos implementar a nivel de interfaces")
}

"appctx.html"(title:"Configuración del AppCtx"){
  "appctx.html"(title:"Declaración de beans y configuración con XML")
  "appctx.html"(title:"Inyección por constructor")
  "appctx.html"(title:"Inyecciones por setter"){
    "appctx.html"(title:"Proceso de resolución de dependencias")
    "appctx.html"(title:"Namespaces ")
    "appctx.html"(title:"Alcance de los beans y modelos de instanciación")
    "appctx.html"(title:"Autowiring de colaboradores"){
      "appctx.html"(title:"Modos de autowiring")  
    }  
  }
  "appctx.html"(title:"Configuración con Anotaciones"){
    "appctx.html"(title:"Descubriendo beans de forma automática")  
  }
  "appctx.html"(title:"Configuración con JavaConfig"){
    "appctx.html"(title:"Replicando el comportamiento de la configuración basada en XML con Java Config")
    "appctx.html"(title:"Inyección de configuración")
    "appctx.html"(title:"Mezclando configuraciones")  
  }
  "appctx.html"(title:"Configuración con Groovy")
}

"spel_profile.html"(title:"Uso de SpEL y Profiles"){
  "spel_profile.html"(title:"Spring Expression Language")
  "spel_profile.html"(title:"Spring Profiles")
}

"aop.html"(title:"Programación orientada a aspectos"){
  "aop.html"(title:"Conceptos esenciales"){
    "aop.html"(title:"JoinPoint")
    "aop.html"(title:"Pointcut")
    "aop.html"(title:"Expresiones")  
  }
  "aop.html"(title:"Declarando aspectos con Anotaciones"){
    "aop.html"(title:"Soporte de anotaciones con AspectJ")
    "aop.html"(title:"Declaración de aspectos(Advice)")
    "aop.html"(title:"Before")
    "aop.html"(title:"After")
    "aop.html"(title:"After returning")
    "aop.html"(title:"After throwing")
    "aop.html"(title:"Around ")  
  }
  
  "aop.html"(title:"JoinPoint en detalle")
  "aop.html"(title:"Declarando aspectos con XML")
}

"agradecimiento.html"(title: "Agradecimiento")
